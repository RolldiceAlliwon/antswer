...
## 레벨 에디터: 절차적 레벨 관리 시스템

대규모 레벨 작업은 협업을 위해 작업 단위를 세분화하여 동시에 진행할 필요가 있습니다. 
언리얼에서는 퍼시스턴트 레벨(Persistent Level)을 중심으로 서브레벨을 구성하고, 각 서브레벨을 스트리밍 방식으로 로드해 이를 구현합니다.

### 레벨 스트리밍

레벨 스트리밍의 목적은 **메모리 로드와 언로드를 제어하는 것**입니다.
